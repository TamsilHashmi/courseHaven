...
export const updateCourse = async (req, res) => {
  ...
  try {
    ...
    if (courseSearch.creatorId.toString() !== adminId.toString()) {
      return res.status(403).json({ errors: "Unauthorized to update this course" });
    }
    ...
    if (req.files && req.files.image) {
      ...
      if (!allowedFormat.includes(image.mimetype)) {
        return res
          .status(400)
          .json({ errors: "Invalid file format. Only PNG, JPG, and WEBP are allowed" });
      }
      ...
      // Optional: destroy the old image on Cloudinary
...
export const courseDetails = async (req, res) => {
  ...
  try {
    ...
    if (!course) {
      return res.status(404).json({ errors: "Couse not found" });
    }
